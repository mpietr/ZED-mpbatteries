...
This report presents an analysis of battery materials from the Materials Project dataset. It explores the attributes, their features and how they are correlated to each other. It attempts to summarize the trends in battery research, which has been more prominent since the popularization of electric vehicles. Regression models are build to predict the Gravimetric Capacity of batteries based on other attributes. They also serve as an experiment to see the significant impact of outliers in the dataset.
...
The data consists of columns:
  ...
  - Volumetric Energy: energy density with respect to battery volume (Wh/L),
...
Above interactive graphs (hosted on [shinyapps.io](https://mpietr.shinyapps.io/mp_batteries/)) show the density of the rest of the numerical attributes. By looking at the shape of the graphs, we can observe that there are outstanding outliers present. 
...
The popularization of electrical vehicles (EVs) has increased the demand for fast-charging and high-energy batteries with as high cycle life as possible. The growing environmental awareness in society also contributes to the priority of battery materials research. Scientists explore various new ideas of battery components to imporve their performance in different applications.

New possibilities of working ions such as Potassium, Magnesium, Aluminum, Sodium are explored as an alternative to more traditional battery chemistries, like in the case of Lithium based batteries. The research aims to help find batteries with improved properties as well as to mitigate ethical, environmental and supply chain risks.

Solid state batteries are being developed. In these batteries liquid electrolyte is replaced with a solid electrolyte, potentially improving energy density and lifespan, but most importantly decreasing the possibility of inflammation. This is especially vital in the EVs and customer electronics industries. 

# Prediction

## Attribute selection

Based on the analysis of the attributes and their correlations, I have made the choice to predict Gravimetric Capacity based on Atomic Friction Discharge, Gravimetric Energy and Max Delta Volume.
...
The model is repeatedly cross-validated in the process of training.

```{r training}
ctrl <- trainControl(method = "repeatedcv", number = 5, repeats = 10)

model <- lm(Gravimetric.Capacity ~ ., data = train)
...
rmse <- sqrt(mean((predictions - test$Gravimetric.Capacity)^2))
...
# Step 4: Plot Predicted vs Actual values using ggplot2 
ggplot(plot_data, aes(x = actual, y = predicted)) +
  geom_point(color = "darkblue") +          # scatter plot for actual vs predicted
...
The Root Mean Square Error of above 100 seems quite high considering the fact that it is close to 1st Quantile (88) and Median (131).
...
model <- lm(Gravimetric.Capacity ~ ., data = new_train)

summary(model)

new_predictions <- predict(model, newdata = new_test)

rmse <- sqrt(mean((new_predictions - new_test$Gravimetric.Capacity)^2))
...
The removal of outliers significantly improved the RMSE. The graph is more concentrated and the model could better adjust to the values of the attributes.

## Conclusions

It is possible to predict Gravimetric Capacity based on Atomic Friction Discharge, Gravimetric Energy and Max Delta Volume, but the model does not perform well with outliers, which make a significant part of the dataset. 